...
public class Bank {
	...
	public Hashtable<String, Account> getAccountlist() {//added for convenience
		return accountlist;
	}

	public boolean accountExists(String acc){ //added this method for OpenAccount method test
		return accountlist.containsKey(acc);
	}
	 ...
	 * @param key The ID of the account
	 * @throws AccountExistsException If the account already exists
	 */
	public void openAccount(String key, Account acc) throws AccountExistsException { //should add accounts to the account list
		if (accountlist.containsKey(key)) {
			throw new AccountExistsException();
		}
		else {
			accountlist.put(key, acc);
		}
	}
	 ...
	 */
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {//this was bullshit, sends exception if account exists
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			accountlist.get(accountid).withdraw(money); //simplified and changed deposit to withdraw
		}
	}
